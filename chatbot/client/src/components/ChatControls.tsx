...
const ChatControls: React.FC<Props> = ({ onChangeMode, vision = false }) => {
  ...
  const handleSendWithDocument = async () => {
    if (!uploadedFile || !rtviClient || processingAction) return;

    // Emit a user message that includes a document preview and the prompt.
    emitter.emit("userTextMessage", [
      { type: "text", text: `Document: ${uploadedFile.name}` },
      { type: "text", text }
    ]);
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_SERVER_URL}/bot/process_data/`,
        { method: "POST", body: formData }
      );

      if (response.ok) {
        const jsonResponse = await response.json();
        const message = jsonResponse.output;
        // Emit the bot's response with a role property so it appears correctly.
        emitter.emit("botTextMessage", [{ type: "text", text: message }]);
        setText("");
  ...
  const handleSubmit = async (ev: FormEvent<HTMLFormElement>) => {
    ev.preventDefault();
    if (uploadedFile) {
      await handleSendWithDocument();
    } else {
      await handleTextSubmit(ev);
    }
  };
      ...
      <div className="bg-secondary rounded-3xl flex flex-col gap-1 p-2">
        ...
        >
          ...
          <div className="flex gap-2">
            {uploadedFile ? (
              <Button
                className="flex-none bg-background rounded-full"
                disabled={!webrtcEnabled || isUploadingFile || processingAction}
                size="icon"
                variant="outline"
                type="submit"
              >
